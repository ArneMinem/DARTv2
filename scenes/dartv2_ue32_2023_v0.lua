...
function motor_curve(vin)
    vout = 0.0
    vstart = 20.0 -- 70  , ! must be changed also in  defineLeftSpeed and defineRightSpeed
    if vin >= vstart then
        a = vstart - 10.0
        b = 1.0
        v = vin
        ...
        vmn = math.log(vstart - a + b)
        -- vout = vstart + (v - vmn) * (255.0 - vstart) / (vmx - vmn)
        vout = (v - vmn) * 255.0  / (vmx - vmn)
    end
    return vout
end
...
function defineLeftSpeed(cmd)
    cmd1 = cmd
    vstart = 20
    if cmd1 < 0 then cmd1 = -cmd1 end
    if cmd1 < vstart then cmd1 = 0 end
    if cmd1 > 255 then cmd1 = 255 end
    cmd1 = motor_curve(cmd1)
    vrepCmd = cmd1 * cvtCmdSpeed
    if cmd < 0 then vrepCmd = -vrepCmd end
    return vrepCmd
end
...
threadFunction = function()
    while simGetSimulationState() ~= sim_simulation_advancing_abouttostop do
        ...
        local dataOut = {
            ...
            distFrontRight, frontLeftEncoder, frontRightEncoder,
            rearLeftEncoder, rearRightEncoder, heading, rb[1], rb[2], rb[3],
            accel[1], accel[2], accel[3], gyro[1], gyro[2], gyro[3],
            rearEncoderLeftReset, rearEncoderRightReset
        }
        ...
        for iw = 1, #wheelId do
            ...
            -- if iw == 1 then print('angle', relativeOrientation) end
        ...
        if not serverOn then
            ...
            if (srv == nil) then
                ...
                simAddStatusbarMessage("server ok at " .. ip .. " on port " ..
                                           port)
                serverOn = true
            ...
            if clt1 == nil then
                ...
                if dataIn ~= nil then
                    ...
                    if targetCmd[5] ~= 0.0 or targetCmd[6] ~= 0.0 then
                        print("received", targetCmd[1], targetCmd[2],
                              targetCmd[3], targetCmd[4], targetCmd[5],
                              targetCmd[6])
                    end
                    ...
                    if speedCmdNew == 1 then
                        ...
                        simSetJointTargetVelocity(frontRightMotor,
                                                  rightWheelSpeed)
                        simSetJointTargetVelocity(frontLeftMotor, leftWheelSpeed)
                        simSetJointTargetVelocity(rearRightMotor,
                                                  rightWheelSpeed)
        ...
        if cnt == cntDispl then
            cnt = 0
            local stOut =
                "Sonars F:" .. distFront .. " R:" .. distRear .. " L:" ..
                    distLeft .. " R:" .. distRight .. " FL:" .. distFrontLeft ..
                    " FR:" .. distFrontRight
            stOut = stOut .. ", Front Odos L:" .. dataOut[8] .. " R:" ..
                        dataOut[9]
            stOut = stOut .. ", Rear Odos L:" .. dataOut[10] .. " R:" ..
                        dataOut[11]

            stOut = stOut .. ", Head:" .. heading .. ", Motors L:" ..
                        leftWheelSpeed .. " R:" .. rightWheelSpeed ..
                        ", Reset L:" .. rearEncoderLeftReset .. " R:" ..
                        rearEncoderRightReset
            stOut = string.format(
                        "Sonar Fr:%.2f Rr=%.2f Le:%.2f Ri:%.2f FL:%.2f FR:%.2f Front Odos L:%.1f R:%.1f Rear Odos L:%.1f R:%.1f Heading: %.2f Motors L:%.2f R:%.2f",
                        distFront, distRear, distLeft, distRight, distFrontLeft,
                        distFrontRight, dataOut[8], dataOut[9], dataOut[10],
                        dataOut[11], heading, leftWheelSpeed, rightWheelSpeed)
            print(stOut)
...
if not res then simAddStatusbarMessage('Lua runtime error: ' .. err) end
